...
// send audio data
// open translation
// get active/inactive translations
// get transcript
// show controls
...
class Controls {
    ...
    listeners() {
        ...
        this.microphoneSelect.addEventListener('click', (e) => {
            const activeMic = this.microphoneSelect.getSelected();
            const mics = this.microphoneSelect.getItems();
            console.log(mics)
            mics.forEach((mic) => {
                mic.addEventListener('click', (e) => {
                    const deviceId = e.target.getAttribute('data-device-id');
                    if (!activeMic) {
                        this.selectMic(deviceId);
    ...
    socketListeners() {
        ...
        socket.on('microphone-list', (data) => {
            console.log(`Received microphone list:`, data);
            this.updateMicrophoneList(data);
        });
        ...
        socket.on('status', (data) => {
            ...
            const paused = serverInfo.find('.paused');
            ...
            paused.find('.status').text('Paused:');
            const pauseLightbulb = paused.find('.material-icons');
            this.toggleLightbulb(pauseLightbulb, data.isPaused ? 'paused' : data.isRunning ? 'not paused' : data.isPaused);
        })
    }
    ...
    start() {
        if (this.selectedMic !== null || this.selectedMic !== -1) {
            this.emit('start');
        } else {
    ...
    getMicrophones() {
        fetch('/microphones')
            .then((res) => {
                console.log(`Received microphone list: ${JSON.stringify(res)}`);
                return res.json();
            })
            .then((data) => {
                console.log(`Received microphone list:`, data);
    ...
    updateMicrophoneList(data) {
        if (data?.devices) {
            // create a dropdown with external data data.devices
            const micContent = this.microphoneSelect.querySelector('dropdown-content');
            console.log(this.microphoneSelect);
            console.log(micContent)
            micContent.innerHTML = '';
            data.devices.forEach((device, el) => {
                console.log('Device:', el, device)
                const mic = document.createElement('dropdown-item');
                mic.setAttribute('data-device-id', device.id);
                mic.setAttribute('value', device.name)
                mic.setAttribute('data-value', device.id);
                mic.textContent = device.name;
                micContent.appendChild(mic);
            });
            // this.microphoneSelect.appendValue(data.devices[0].name, {
            //     value: data.devices[0].deviceId,
    ...
    selectMic(mic) {
        ...
        this.microphoneSelect.selectItem(mic);
        this.emit('set_device_index', mic);
    }
}
